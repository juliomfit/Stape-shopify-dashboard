...
test("production freshness crons are independent and not sequential sync-all", () => {
  ...
  assert.match(daily, /runEveningIngest/);
  ...
  const evening = readFileSync("src/app/api/cron/evening/route.ts", "utf8");
  assert.match(evening, /handleSourceCron\(request, "evening"\)/);
  assert.doesNotMatch(evening, /"all"/);
});
