...
WITH purchases AS (
  SELECT
    ...
    LOGICAL_OR(REGEXP_CONTAINS(IFNULL(page_location, ""), r"[?&]utm_content=")) AS has_utm_content
...
SELECT
  ...
  (SELECT COUNTIF(has_utm_campaign) FROM purchases) AS with_utm_campaign,
...
-- raw_events_full typically has fbclid (column and/or URL), not Meta campaign_id.
-- utm_campaign is read from page_location; it is not assumed as a table column.
