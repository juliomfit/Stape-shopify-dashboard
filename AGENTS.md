...
- Pacific “today” page_view check (54 rows): `url_has_gclid=0` and `gclid_column_filled=0` — no Google Ads landings in that sample, so gclid 0% is not a writer bug yet. Meta: `url_has_fbclid=16` vs `fbclid_column_filled=18` means the column is filling from URL plus first-touch cookie. Re-check gclid after a day with `gclid=` in `page_location`; only then treat empty `gclid` as a server GTM field-map issue.
