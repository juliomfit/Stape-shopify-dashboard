...
- Meta **channel** attribution may exist without campaign mapping. Exact first-party IDs (`gn_meta_campaign_id` / `gn_meta_adset_id` / `gn_meta_ad_id`) are authoritative HIGH joins. Campaign-name fallback is legacy PARTIAL only. Never fuzzy-map Meta campaigns. Never infer ad/adset IDs from aggregate Meta facts. Unmapped Meta credit must remain visible. Adset/ad OUR metrics stay hidden until exact IDs exist. Do not modify canonical attribution math for Meta identity enrichment. Warehouse SQL extracts IDs from `page_location`; do not reference typed `raw_events_full.meta_*` columns until migration 006 is confirmed. Web GTM Meta-ID patch is PREPARED FOR IMPORT (`docs/GTM_MANUAL_CHANGES.md`). Ads Manager URL contract: `docs/META_ATTRIBUTION_SETUP.md` (MANUAL META UI VERIFICATION REQUIRED for `{{campaign.id}}` tokens). Historical unmapped Meta touches stay unmapped.
