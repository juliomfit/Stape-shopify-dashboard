...
- Attribution Models / Journeys / Attribution Overview accept `?lookback=1|7|14|30|60` and (where relevant) `?model=`. Default window is 7 days until conversion-lag validation (`bigquery/validation/11_conversion_lag_distribution.sql`). 90-day is hidden until raw_events_full retention covers it.
...
- Do not invent Meta campaign-level nCAC. Campaign OUR revenue joins only on campaign id/name match; mapping coverage is VALIDATION REQUIRED until Julio runs `bigquery/validation/05_meta_campaign_mapping_coverage.sql`.
...
- Click-id fill is **live as of 2026-08-15** on both `Data Client` and `GA4` copies of `page_view` in `raw_events_full`. Dual GA4 + Data Client rows are expected. **No Google Ads account for now**. Residual `gclid` may be leftover URLs or Shopify Shop campaigns. Keep mapping `gclid`. Do not build a Google Ads product. Paid traffic to diagnose is Meta/`fbclid`.
