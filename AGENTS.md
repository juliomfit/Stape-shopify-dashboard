...
- Click-id fill (Pacific, `page_view` on `raw_events_full`): **today** 54 rows, `url_has_gclid=0` / `gclid_column_filled=0`, `url_has_fbclid=16` / `fbclid_column_filled=18` (Meta column ≥ URL = first-touch cookie working). **Last 7 days** 4014 rows, `url_has_gclid=6` / `gclid_column_filled=4`, `url_has_fbclid=2396` / `fbclid_column_filled=1562`. Google Ads volume is tiny. The 7-day Meta URL>column gap is likely pre-stitch days in the window; do not treat it as a live writer failure until a day-by-day overlap (`url has id AND column empty`) shows the miss still happening after stitch. Do not ask Julio to re-publish web GTM. If recent days still miss URL click IDs, the leftover is server `GTM-NJ4QCWFK` field map / which `source_client` copy gets `{{gn_gclid}}` / `{{gn_fbclid}}`.
