...
Hobby cron remains daily (`0 15 * * *` UTC). Use Refresh Meta for on-demand. `POST /api/meta/sync` `maxDuration` is **300s** (needed for `FLYWEEL_INGEST_LEVELS=all`). Vercel Hobby still caps at 60s — Production must allow 300s. Skip BigQuery DELETE while the streaming buffer is hot. Vercel cookie `durable-json` is not a global lock; warehouse `sync_runs` is the concurrency guard.
