...
| Meta Ads | `/meta` | Warehouse campaign facts (Flyweel ingest). Charts never call Flyweel on GET. Ad set/ad facts are unavailable from Flyweel `query_metrics` (campaign grain only). | Platform Ads Manager matching. Same Meta dollars as Overview Meta. |
